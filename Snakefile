    ...
    run:
        ...
        if not (os.path.exists(os.path.join(output_dir, "AMR_alleles.fa")) and os.path.exists(os.path.join(output_dir, "AMR_headers.txt"))):
            ...
            for database in tqdm(["ncbi"]):#["argannot",
                ...
                #shell(f"cp /hps/nobackup/iqbal/dander/amira_panRG_pipeline/Escherichia_coli_panRG_c_0.8_l_0_train_AMR_alleles_removed_old/ariba_downloads/* {os.path.dirname(db_output)}")
...
rule list_bakta_gffs:
    ...
    run:
        # list all gff files output by bakta
        gff_files = glob.glob(os.path.join(os.path.dirname(input[0]), "*", "*.gff*")) + [input[-3]]
